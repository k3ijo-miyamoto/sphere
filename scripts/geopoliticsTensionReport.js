...
function parseArgs(argv) {
  const out = {
    compareFrom: 0,
    output: "geopolitics_tension_report.json",
    top: 5
  };
...
async function main() {
  ...
  const report = await callStateTool("sphere_geopolitics_report", {
    compareFrom: Math.max(0, opts.compareFrom),
    top: Math.max(1, opts.top)
  });
